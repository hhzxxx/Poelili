<template>
  <el-dialog :model="show"
             title="新增">
    <el-form ref="form"
             :model="sdata">
      <el-form-item size="mini"
                    label="Serve">
        <el-select fit-input-width="true"
      ...
      <el-form-item size="mini"
        ...
        <el-select fit-input-width="true"
                   ...
                   placeholder="league">
          <el-option v-for="(item, index1) in leagues[sdata.domain]"
                     :key="index1"
                     :label="item.id"
      ...
      <el-form-item v-if="!add"
        ...
        <el-input :disabled="add"
                  ...
                  placeholder="minPrice"></el-input>
      </el-form-item>
      <el-form-item v-if="!add"
                    label="最高价">
        <el-input :disabled="add"
                  v-model="sdata.maxPrice"
                  placeholder="maxPrice"></el-input>
      </el-form-item>
      <el-form-item v-if="!add"
                    label="排序">
        <el-select :disabled="add"
                   @change="changeSort"
                   fit-input-width="true"
                   v-model="sort"
                   placeholder="sort">
          <el-option label="最近"
                     value='{"indexed":"desc"}'></el-option>
          <el-option label="低价"
                     value='{"price":"asc"}'></el-option>
          <el-option label="高价"
                     value='{"price":"desc"}'></el-option>
        </el-select>
      </el-form-item>
    </el-form>
    <template #title>
      <span class="dialog-footer">
        <h3>新增</h3>
        <el-button v-if="sdata.league&&sdata.domain"
                   style="float: right"
                   type="primary"
                   @click="newWindow">网页</el-button>
      </span>
    </template>
  </el-dialog>
</template>
