...
export class Window {
	...
	// 获取BrowserView
	getView(id) {
		return BrowserView.webContents.fromId(id)
	}

	// 获取全部BrowserView
	getAllViews() {
		return BrowserView.webContents.getAllWindows()
	}
	...
	createWindows(options) {
		...
		if (process.env.WEBPACK_DEV_SERVER_URL) {
			...
			winURL = args.route
				? `http://localhost:9090${args.route}`
	...
	createTray() {
		...
		const trayIco = path.join(__dirname, '../static/timg.jpg')
		console.log(trayIco)
	...
	listen() {
		...
		ipcMain.on('window-max', (event, winId) => {
			if (winId) {
				if (this.getWindow(Number(winId)).isMaximized()) {
					// 为true表示窗口已最大化
					this.getWindow(Number(winId)).restore() // 将窗口恢复为之前的状态.
				} else {
				...
				for (let i in this.group) {
					if (this.group[i]) {
						if (this.getWindow(Number(i)).isMaximized()) {
							// 为true表示窗口已最大化
							this.getWindow(Number(i)).restore() // 将窗口恢复为之前的状态.
						} else {
		...
		// 创建view
		ipcMain.on('view-new', (event, args) => {
			this.createViews(args).then((res) => {
				event.sender.send('view-reply', res)
				event.reply('view-reply', res)
			})
		})

		// 关闭
		ipcMain.on('view-closed', (event, winId) => {
			if (winId) {
				this.getWindow(Number(winId)).close()
				if (this.group[Number(winId)]) delete this.group[Number(winId)]
			} else {
